...
# Five pilot Notes were synthetic numeric-application duplicates. Their stable
# IDs remain reserved and are not reused in the production batch.
...
def main() -> int:
    ...
    for row_no, row in enumerate(notes, start=2):
        ...
        if indices != {1}:
            fail(
                errors,
                f"{note_id}: v1.2 COM-01 requires one generated card; "
    ...
    if seen_ids & RESERVED_PILOT_ONLY_IDS:
        fail(errors, "reserved pilot-only Note IDs are present")
    ...
    if errors:
        print("COM-01 v1.2 production validation: FAIL", file=sys.stderr)
        for error in errors:
    ...
    print("COM-01 v1.2 production validation: PASS")
    print(
