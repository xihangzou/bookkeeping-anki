#!/usr/bin/env python3
"""Validate IND-10 production Notes under the current living recall rules."""
...
NOTES = ROOT / "production" / "notes" / "IND-10.tsv"
INVENTORY = ROOT / "inventory" / "topic_inventory" / "IND-10.tsv"
FIELDS = [
...
EXPECTED_ALP_MAP = {
    f"BK-IND-10-{n:04d}": [f"ALP-IND-10-{n:04d}"] for n in range(1, 19)
}

REQUIRED = {
    "BK-IND-10-0001": ("{{c1::年次決算}}", "{{c1::月次決算}}"),
    "BK-IND-10-0002": ("{{c1::売上}}", "{{c1::売上原価}}", "{{c1::販売費及び一般管理費}}", "{{c1::年次損益}}"),
    "BK-IND-10-0003": ("借方に{{c1::売上原価}}", "{{c1::販売費及び一般管理費}}", "貸方に{{c1::売上}}"),
    "BK-IND-10-0004": ("12か月分の{{c1::営業損益}}", "{{c1::営業外損益}}", "{{c1::特別損益}}", "{{c1::当期純利益}}"),
    "BK-IND-10-0005": ("月次決算を行わない場合", "{{c1::年次決算}}"),
    "BK-IND-10-0006": ("{{c1::販売費}}", "{{c1::一般管理費}}"),
    "BK-IND-10-0007": ("損益計算書・貸借対照表に加え", "{{c1::製造原価報告書}}"),
    "BK-IND-10-0008": ("{{c1::期首製品棚卸高}}＋{{c1::当期製品製造原価}}－{{c1::期末製品棚卸高}}",),
    "BK-IND-10-0009": ("不利差異は{{c1::加算}}", "有利差異は{{c1::減算}}"),
    "BK-IND-10-0010": ("売上総利益＝{{c1::売上高}}－{{c1::売上原価}}", "営業利益＝売上総利益－{{c1::販売費及び一般管理費}}"),
    "BK-IND-10-0011": ("{{c1::原材料}}", "{{c1::仕掛品}}", "{{c1::製品}}"),
    "BK-IND-10-0012": ("材料勘定の{{c1::次期繰越額}}", "仕掛品勘定の{{c1::月末仕掛品原価}}", "製品勘定の{{c1::月末製品原価}}"),
    "BK-IND-10-0013": ("{{c1::製造原価報告書}}",),
    "BK-IND-10-0014": ("{{c1::当期総製造費用}}＋{{c1::期首仕掛品原価}}－{{c1::期末仕掛品原価}}",),
    "BK-IND-10-0015": ("{{c1::期首材料棚卸高}}＋{{c1::当期材料仕入高}}－{{c1::期末材料棚卸高}}",),
    "BK-IND-10-0016": ("{{c1::材料費}}・{{c1::労務費}}・{{c1::経費}}", "{{c1::製造直接費}}・{{c1::製造間接費}}"),
    "BK-IND-10-0017": ("原価差異を除いた{{c1::予定価格}}ベース", "原価差異を反映した{{c1::実際価格}}ベース"),
    "BK-IND-10-0018": ("{{c1::仕掛品}}へ集計", "{{c1::製品}}へ振替", "{{c1::売上原価}}へ振替", "{{c1::売上原価}}へ反映"),
}
...
def main() -> int:
    ...
    if errors:
        print("IND-10 production validation: FAIL")
        for error in errors:
            print("-", error)
        return 1
