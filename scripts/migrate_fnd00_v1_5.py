...
TEXT_OVERRIDES = {
    ...
    "BK-FND-00-0048": "立替金・預り金では、一時立替額の回収権は {{c1::資産}}、一時的な返還義務は {{c1::負債}}。給料日前の前貸しは {{c1::従業員立替金}} で処理し、給与から前貸し分を回収する場合も給与は {{c1::総額}} を費用計上する。預り金は受入時に {{c1::貸}}方、返還時に {{c1::借}}方で処理する。",
    ...
    "BK-FND-00-0062": "主要簿では、取引を発生順に記録する {{c1::仕訳帳}} と、科目別に記録する {{c1::総勘定元帳}} を使う。前者の参照欄 {{c1::元丁}} は転記先、後者の参照欄 {{c1::仕丁}} は転記元を示す。",
    ...
    "BK-FND-00-0084": "伝票からの転記では、各伝票からその都度行うのが {{c1::個別}}転記、一定期間分を集計して行うのが {{c1::合計}}転記。1日・1週・1月単位で用いる表は {{c1::仕訳日計表}}・{{c1::週計表}}・{{c1::月計表}} で、後者の流れは伝票→{{c1::集計表}}→{{c1::総勘定元帳}}。補助元帳は各伝票からその都度転記する。",
...
ALP_EXACT = {
    "BK-FND-00-0048": "ALP-FND-00-0043 ALP-FND-00-0044 ALP-FND-00-0045 ALP-FND-00-0046",
    "BK-FND-00-0084": "ALP-FND-00-0080 ALP-FND-00-0081 ALP-FND-00-0082 ALP-FND-00-0083",
    "BK-FND-00-0091": "ALP-FND-00-0090 ALP-FND-00-0091",
}
SECTION_OVERRIDES = {
    "BK-FND-00-0004": "2.1 簿記の5要素",
    "BK-FND-00-0009": "4.1 会計期間",
    "BK-FND-00-0075": "10.6 売上帳・仕入帳",
}
REACTIVATE = {"BK-FND-00-0048", "BK-FND-00-0084", "BK-FND-00-0091"}
...
def main() -> int:
    ...
    referenced = set(TEXT_OVERRIDES) | set(ALP_ADDITIONS) | set(ALP_REMOVALS) | set(ALP_EXACT) | set(SECTION_OVERRIDES) | REACTIVATE
    ...
    for note_id, removals in ALP_REMOVALS.items():
        current = set(by_id[note_id]["ALP_IDs"].split())
        by_id[note_id]["ALP_IDs"] = " ".join(sorted(current - removals, key=alp_key))
    for note_id, additions in ALP_ADDITIONS.items():
        current = set(by_id[note_id]["ALP_IDs"].split())
        by_id[note_id]["ALP_IDs"] = " ".join(sorted(current | additions, key=alp_key))
    for note_id, alp_ids in ALP_EXACT.items():
        by_id[note_id]["ALP_IDs"] = alp_ids
    for note_id, section in SECTION_OVERRIDES.items():
        by_id[note_id]["Section"] = section
    ...
    for note_id in REACTIVATE:
        ...
        if "ANKI-AUDIT-005" not in row["Extra"]:
            row["Extra"] = (row["Extra"].rstrip() + " ANKI-AUDIT-005で関連ALPを1枚へ統合してactive direct recallへ復帰。").strip()
    ...
    if len(approved) != 32 or cards != 32 or spans != 120 or len(active_alps) != 91:
